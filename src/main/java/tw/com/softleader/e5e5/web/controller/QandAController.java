...
public class QandAController {
	...
	public @ResponseBody List<QuestionAndAnswer> postanswer(@RequestBody Answer answer, Model model) {
		if (answer.getAnswer().length() > 0) {
			qandaservice.updateAnswer(answer.getAnswer(), answer.getId());
		}
		List<QuestionAndAnswer> qandas = qandaservice.findByProductId(answer.getProductid());
		return qandas;
	}
}//end of controller
